...
// //////////////////////////////////////////////////////////////////////////
#include "..\..\stdafx.h"
#include "..\..\Include\Utils\xstatus4.h"
...
BOOL COXStatusBar::SetPaneCursor(int nIndex, UINT nCursorID /*=0 */)
	...
	if (hNewCursor != NULL)
		{
		m_CursorArray[nIndex]=(DWORD)hNewCursor;
		}
	else
		{
	...
	for (int i=0; i < m_nCount; i++)
		...
		if (!(pSBP->nStyle & SBPS_DISABLED))
			...
			if(((CBmpInfo*)m_PaneBmp[i]) != NULL)
				...
				DrawStatusBmp(&DC, i, pSBP->nStyle);
			}
		}
	}
								  ...
								  DWORD dwTotal /*=100 */, DWORD dwDone /*=0 */)
{
	...
	if (pEvolInfo->m_bPercentText)
	{
		...
		TCHAR tmpBuf[5];
		_stprintf(tmpBuf,_T("%d%%"),pEvolInfo->m_nPercentDone);
...
BOOL COXStatusBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
	...
	if (nHitTest==HTCLIENT)
		...
		CPoint point(LOWORD(dw), HIWORD(dw));
		...
		if (i < m_nCount && m_CursorArray[i] != NULL)
			{
			::SetCursor((HCURSOR)m_CursorArray[i]);
